...
const ProjectModal = ({ project, onClose }) => {
  return (
    <motion.div
      ...
      className="fixed inset-x-0 bottom-0 z-[200] h-[85vh] bg-white rounded-t-[2.5rem] shadow-2xl flex flex-col overflow-hidden border-t border-gray-200"
    >
      <div className="flex justify-between items-center p-8 border-b border-gray-100 bg-white">
        <div>
           ...
           <h2 className="text-3xl md:text-4xl font-bold text-black mt-2 tracking-tight">{project.title}</h2>
        </div>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20">
          ...
          <div className="space-y-10">
             <div>
               ...
               <p className="text-xl text-gray-700 leading-relaxed">{project.fullDesc}</p>
             </div>
             <div>
               ...
               <div className="border-l-4 border-green-500 pl-6 py-1">
                 <p className="text-2xl text-black font-medium italic">"{project.impact}"</p>
               </div>
             </div>
          </div>
        </div>
...
const Hero = () => {
  return (
    <div className="min-h-[90vh] flex flex-col justify-center px-6 md:px-20 bg-white relative overflow-hidden">
...
const HorizontalGallery = ({ onProjectClick }) => {
        ...
        <motion.div style={{ x }} className="flex gap-8 md:gap-16 px-6 md:px-20">
          {DATA.projects.map((project) => (
            <div 
              ...
              className="group relative h-[60vh] w-[85vw] md:w-[60vw] flex-shrink-0 bg-white rounded-3xl overflow-hidden shadow-2xl hover:shadow-3xl transition-all duration-500 cursor-pointer border border-gray-100"
            >
              ...
              <div className="absolute bottom-0 left-0 w-full p-8 md:p-12 text-white">
                 <div className="flex justify-between items-end">
                    <div>
                       ...
                       <h3 className="text-3xl md:text-5xl font-bold tracking-tight mb-4">{project.title}</h3>
                       <p className="text-gray-300 max-w-lg text-lg line-clamp-2 md:line-clamp-none">{project.desc}</p>
                    </div>
                    <div className="hidden md:flex h-14 w-14 bg-white text-black rounded-full items-center justify-center group-hover:scale-110 transition-transform duration-300"><ArrowUpRight size={24}/></div>
                 </div>
              </div>
            </div>
          ))}
    ...
    <section className="py-32 px-6 md:px-20 bg-white">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-20">
        <div>
           ...
           <h2 className="text-4xl md:text-6xl font-bold text-black mb-8 leading-tight">Bridging technical engineering and strategic vision.</h2>
...
export default function LightModePortfolio() {
  ...
  return (
    <div className="bg-white text-black font-sans selection:bg-black selection:text-white">
      ...
      <Hero />
      <HorizontalGallery onProjectClick={setSelectedProject} />
      <About />
      <div className="h-[60vh] bg-black text-white flex flex-col items-center justify-center text-center px-6">
        <h2 className="text-5xl md:text-7xl font-bold tracking-tighter mb-8">Ready to collaborate?</h2>
